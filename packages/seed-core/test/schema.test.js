...
import { registryFromJson, registryToJson, Registry, validateRecord, } from "../src/index.js";
const usersSchema = {
    name: "users",
    version: "1",
    source: "code",
...
describe("validateRecord", () => {
    it("passes a valid record", () => {
        expect(() => validateRecord(usersSchema, { email: "alice@example.com", role: "user" })).not.toThrow();
    });
    it("flags missing required field", () => {
        expect(() => validateRecord(usersSchema, { email: "alice@example.com" })).toThrow(/E_CONSTRAINT_TYPE/);
    });
    it("flags enum violation", () => {
        expect(() => validateRecord(usersSchema, {
